...
public class ScaleTest extends JComponent {
	...
	private static BufferedImage originalImage = new BufferedImage(FULL_SIZE,
			FULL_SIZE, BufferedImage.TYPE_INT_RGB);
	 ...
	 * quadrants of this image: RGB stripes, a picture, vector art, and a black
	 * and white grid.
	 */
	private void paintOriginalImage() {
		...
		g.drawArc(20, QUAD_SIZE + 20, (QUAD_SIZE - 40), QUAD_SIZE - 40, 190,
				160);
		int eyeSize = 7;
		int eyePos = 30 - (eyeSize / 2);
		g.fillOval(eyePos, QUAD_SIZE + eyePos, eyeSize, eyeSize);
		g.fillOval(QUAD_SIZE - eyePos - eyeSize, QUAD_SIZE + eyePos, eyeSize,
				eyeSize);
	...
	private void drawBetterImage(Graphics g, int yLoc) {
		...
		for (int scaledSize = FULL_SIZE; scaledSize > 0; scaledSize -= delta) {
			Image scaledImage = getOptimalScalingImage(originalImage,
					FULL_SIZE, scaledSize);
	...
	/**
	 * This approach uses either the getScaledInstance() approach to get each
	 * new size or it scales on the fly using drawImage().
	 */
	private void drawImage(Graphics g, int yLoc, boolean getScaled) {
		...
		if (getScaled) {
			for (int scaledSize = FULL_SIZE; scaledSize > 0; scaledSize -= delta) {
				...
				g.drawImage(scaledImage, xLoc, yLoc + (FULL_SIZE - scaledSize)
						/ 2, null);
				xLoc += scaledSize + 20;
			}
		} else {
			for (int scaledSize = FULL_SIZE; scaledSize > 0; scaledSize -= delta) {
				g.drawImage(originalImage, xLoc, yLoc
						+ (FULL_SIZE - scaledSize) / 2, scaledSize, scaledSize,
						null);
				xLoc += scaledSize + 20;
			}
		}
	...
	/**
	 * Scale the image to several smaller sizes using each of the approaches and
	 * time each series of operations. The times are output into the application
	 * window for each row that they represent.
	 */
	protected void paintComponent(Graphics g) {
		...
		g.drawString(Long.toString(totalTime) + " ms", xLoc, yLoc
				+ (FULL_SIZE / 2) + 15);
