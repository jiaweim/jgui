...
public class PictureScaler extends JComponent {
	...
	public PictureScaler() {
		try {
			...
			System.out.println("w, h = " + picture.getWidth() + ", "
					+ picture.getHeight());
	...
	/**
	 * Render all scaled versions 10 times, timing each version and reporting
	 * the results below the appropriate scaled image.
	 */
	protected void paintComponent(Graphics g) {
		...
		g.drawString(Float.toString(totalTime) + " ms", xLoc, yLoc + scaleH
				+ PADDING + 10);
		...
		startTime = System.nanoTime();
		for (int i = 0; i < iterations; ++i) {
			Image scaledPicture = getFasterScaledInstance(picture, scaleW,
					scaleH, RenderingHints.VALUE_INTERPOLATION_BILINEAR, true);
			g.drawImage(scaledPicture, xLoc, yLoc, null);
		}
		endTime = System.nanoTime();
		totalTime = (float) ((endTime - startTime) / 1000000) / iterations;
