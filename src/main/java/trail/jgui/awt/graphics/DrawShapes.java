...
public class DrawShapes extends JComponent {
	 ...
	 * parameters. The Shape is created by constructing a GeneralPath that moves
	 * between the inner and outer rings.
	 */
	private static Shape generateStar(double x, double y, double innerRadius,
	 ...
	 */
	private static Shape generateDonut(double x, double y, double innerRadius,
			double outerRadius) {
	 ...
	 * alternately, in the click location. The new shape is added to the List of
	 * current shapes, then the scene is repainted.
	 */
	private class ClickReceiver extends MouseAdapter {
		public void mouseClicked(MouseEvent me) {
			...
			if (getStar) {
				...
				newShape = generateStar(centerX - outerSize, centerY
						- outerSize, innerSize, outerSize, numPoints);
			} else {
				newShape = generateDonut(centerX - outerSize / 2, centerY
						- outerSize / 2, innerSize, outerSize);
			}
			getStar = !getStar;
			shapes.add(newShape);
			repaint();
